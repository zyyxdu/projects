    ...
    wx.showToast({
      ...
      success:function(){ 
        that.setData({
         ...
         db.collection("user_reserve").doc(that.data._id).update({
          data:{
            is_approve: 0,
            // state:1,
            // time_approve:DATE
          ...
          .get({
            ...
            wx.cloud.callFunction({
              ...
              data:{
                ...
                name:res.data.name,
                result:"注册失败",
                reason:that.data.reject
              }
        
                
            }).then(res=>{
              console.log("发送注册失败消息成功",res)
            }).catch(err=>{
              console.log("发送注册失败消息失败",err)
            })
          }).catch(err=>{
            console.log(err)
          })
